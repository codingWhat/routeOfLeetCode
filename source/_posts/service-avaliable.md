---
title: 服务可用性-理论篇
date: 2024-07-28 17:52:10
tags:
- GO
- 微服务
- 服务可用性
- 可用性治理
---

# 概念
服务可用性是指`服务正常运行的时间/服务实际运行的时间`, 不过专业来说应该是`可用性=MTTF / (MTTR+MTTF) * 100%`, MTTF(Mean time to failure) 平均无故障时间，MTTR(Mean time to repair)平均故障修复时间,这俩指标后面细说.

## 服务质量指标
主要包含SLA、SLI、SLO、这三个指标分别对应 `协议`、`指标`、`目标`。 
SLA一般是对外的，比如付费业务通常都会制定SLA协议，如果服务不满足协议要求，会有对应的赔偿方案，可参考[腾讯云SLA](https://cloud.tencent.com/document/product/301/103169#63ee1985-f56f-4629-afbf-cafde690ca64)
SLI和SLO关系好比是指标和值的关系，比如`push触达率:99.9%` 即 SLI: push触达率, SLO: 99.9%

# 如何制定质量指标？
在实际工程中，首先需要决策服务可用性采用"几个9"? 比如"99.9%", "99.99%"以及"99.999%"，可用性依次越来越高，至于选几个9不是拍脑袋定的。
## 第一步: 了解自身服务的特点
- 确认系统提供什么类型的服务?
- 确认不同故障的影响如何?
- 确认成本和可用性之间的关系？

### 确认系统提供什么类型的服务?
1. 付费还是免费?
2. 是否对公司收入非常重要?
3. 竞争对手提供什么服务?
4. to B 还是 to C?

### 确认不同故障的影响如何?
1. 确认计划外故障(断断续续故障(快速修复)，还是直接宕机(关闭服务))
2. 计划内故障

### 确认成本和可用性之间的关系？
1. 提升一个9需要付出多少成本，两者之间的权衡


## 第二步: 确定服务质量指标
1. 可用性级别: 是否高可靠
2. 故障类型: 是否能容忍故障
3. 成本: 比如有的客户端要求延时，有的需要吞吐，需要根据不同需求场景，优化成本不一致


## 第三步:制定质量方案
1. 设置SLO、错误预算(裁决：功能迭代 vs  系统稳定)
2. 监控指标, 立体监控

<strong>如何选择SLI</strong>
这里需要说明下，`可用性时间`在某些场景下会产生歧义([SRE-计划外停机](https://sre.google/sre-book/embracing-risk/))，并不会作为SLI出现，相反成功率、请求延时、吞吐、连接数这些指标反而更常见。比如一个长连接网关系统，可能会更加关注连接数、延时、吞吐等
而评论系统可能更加关注吞吐、成功率等。所以SLI指标跟业务的关注点是分不开的。

## 可用性指标
可用性指标和质量指标不太一样更多强调的是时间范围，以MTTF(平均无故障时间)、MTTR(平均故障修复时间)、MTBF(平均故障时间)三个指标为例，如下图我们可以看到这三者的关系

![MTTF、MTTR、MTBF](/images/available_metric.png)



MTTR反映了服务整体的可靠性，MTTR越长服务越可靠。
MTTF衡量可用性，MTTF越长说明系统可用性越好，越稳定。MTTR则代表系统恢复速度，值越小说明系统鲁棒性越好。

MTTR又可以分为:
MTTI(mean time to identify), 平均故障识别时间，这个阶段会通过各个渠道(告警、舆情、用户反馈)收到故障信息
MTTK(mean time to know), 平均故障认知时间，定位业务、定位对应开发、开发定位问题根因所花时间
MTTS(mean time to solve), 平均故障解决时间，这部分主要包含上线修复、验证的时间


# 如何提升可用性?
已知`可用性=MTTF/MTTF+MTTR`, 延长MTTF，缩短MTTR时间。具体来说首先要在事前(故障前)做好"万全"的准备，事中(故障发生时)要提升发现速度+恢复速度,。
## 事前
### 变更规范
- 严格上线流程
- 代码/配置变更
### 代码质量
- 建立代码规范，统一标准
- 严格的code review
### 测试验证
- 做好核心逻辑单测、集成测试
- 自测、系统全链路压测，观察系统在极限场景下的表现，提前检测出隐藏bug或者风险点。
### 容量评估
- 对于提前预知的流量(春节、赛事活动等)，提前做好扩容。
- 预留容量buffer
- 弹性伸缩
### 架构设计
- 异步解耦和削峰设计（消息队列）
- 分层、无状态设计
- 常规治理手段: 可查看《[服务可用性治理](https://codingwhat.github.io/2024/07/17/serive-high-available-governance/)》

## 事中(MTTR)
### 故障发现(MTTI)
1.灰度发布，金丝雀发布，将故障影响降到最低。
2.监控告警，通过立体化监控(可观测)及时发现系统异常(基础资源、服务)
3.外部获取，舆情/客服/反馈平台获取。
### 故障认知(MTTK)
1.告警事件处理流程优化，持续优化监控告警，确保告警的准确性、及时性和可触达性
### 故障解决(MTTS)
- 回滚，发现异常快速回滚线上变更，降低影响面
- 可观测，根据告警信息，跟踪链路，定位错误日志信息
- 扩容，如果发现流量异常，及时扩容
- 容灾切换开关, 比如idc有异常，做流量切换
- 限流开关，针对服务、接口、appid做不同的限流配置。
- 熔断开关, 下游异常时，调整熔断配置，直接拒绝请求，避免资源浪费、堆积
- 降级开关，在发生异常时，可以通过打开开关，走降级逻辑，比如用户标签信息服务异常，可以先展示默认信息。
- 优先级开关，在服务过载时可以调整请求优先级，优先保证高优请求。

## 事后
1.故障复盘: 目标回顾、评估结果、故障影响、原因分析、过程回顾、总结规律、行动计划
2.故障演练, 引入混沌工程，暴露系统隐患


# 参考资料
> [《Google SRE》](https://sre.google/sre-book/embracing-risk/)